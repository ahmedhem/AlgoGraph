...
// Todo
/*
  Try to delete the diprecated code between different popUp also refactor the classes name to be globl and not specifed to a certain popUp
 */
export function openSourcePopup(runAlgo) {
  modal.insert_content(`
        <div class="root-pickup-container">
            <span id="root-pickup-input-text">Type the source</span>
            <input id="root-pickup-input" class="" type="number" min="1"  value="1">
            <button type="submit" class="btn confirm">Confirm</button>
            <span id="root-pickup-error"></span>
  ...
  //..listing for enter clicks in the popup input field
  document.querySelector("#root-pickup-input")
    .addEventListener("keyup", (e) => {
      if (e.keyCode === 13) {
        const value = e.target.value;
        runAlgo(parseInt(value));
        e.target.value = 1;
        modal.close();
      }
    });
